...
Segue abaixo seus pagamentos solicitados
</div>

<table class="table table-condensed">
    <thead style="background-color:#000; color:#fff;">
        <tr>
            <th width="89%" style="text-align:left; vertical-align: middle;">Detalhes</th>
            <th width="10%" style="text-align:right; vertical-align: middle;">Status</th>
            <th width="1%" style="text-align:right; vertical-align: middle;">&nbsp;</th>
        </tr>
    </thead>
    
<?Php
$limite = mysqli_query($conexao, "SELECT * FROM sps_pagamentos WHERE pag_afiliado_id='$id' ORDER BY pag_id DESC LIMIT 30");
...
<tbody style="font-size:12px;">
    <tr>
        <td style="text-align:left; vertical-align: middle; cursor:pointer;">
            #<?php echo $produto['pag_id']; ?> - <?php echo substr($produto['pag_tipo'], 0, 27); ?><br>
            <span class="w3-text-gray" style="font-size:9px;"><?php echo $produto['pag_codigo']; ?><br>
            R$ <?php echo number_format($produto['pag_valor'],2,",","."); ?> - <?php echo $produto['pag_descricao']; ?>
            </span>
        </td>
        <td style="text-align:right; vertical-align: middle;"><?php echo $produto['pag_status']; ?></td>
        <td style="text-align:right; vertical-align: middle;">
            <?Php
                if($produto['pag_id'] == ""){
		        }else{
			        if($produto['pag_status'] == "Realizado"){
			   ?>
                    <a href="pagamento_comprovante.php?id=<?php echo $id; ?>&&idpag=<?php echo $produto['pag_id']; ?>&&empresa=<?Php echo $empresa; ?>" title="Imprimir comprovante"><img src="../img/icon_relatorio.png" width="20px" /></a>
            <?Php
			        }else{
			        }
		        }
