...
$delete = mysqli_query($conexao, "DELETE FROM sps_banco_favorecido WHERE favorecido_id='$favorecido'");

if($delete == "1"){
    echo "<script>location.href='retirar_confirmar_saque.php?favorecido=0&&id=".$id."&&limite=".$limite."&&limite2=5000&&quantidade=".$quatidadedisponivel."&&tarifa=".$servico."';alert('Conta excluída com sucesso');</script>";
}else{
    echo "<script>history.back(-1);alert('Erro ao excluir!');</script>";  
}
?>
</div>
