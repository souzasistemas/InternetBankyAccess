...
<?Php
  ...
  $sqlLojas = mysqli_query($conexao, "SELECT * FROM sps_afiliados WHERE afiliado_credenciamento='Sim' AND afiliado_nacao='$pais' AND afiliado_estado != '' GROUP BY afiliado_estado");
  $verLojas = mysqli_fetch_array($sqlLojas);
  
  if($verLojas['afiliado_id'] == ""){
  ?>
  
  <h1 class="w3-large w3-center">Desculpe! Ainda não estamos atendendo neste País</h1><br>
  ...
  echo " ".strtoupper($pais)."<br><br>";
  ?>
  
  
  <strong>2º Passo</strong><br><br>
    Escolha a Estado ou Província<br><br>
  
  <form action="lojas_estado_cidade.php?id=<?Php echo $id; ?>&&empresa=<?Php echo $empresa; ?>" name="form" method="post">
  
  <select name="estado" class="form-control" style="font-size:20px; text-align:center; height: 50px;" required>
  	<option value=""></value>
  	
  	
  	<?Php
  $sqlLojas2 = mysqli_query($conexao, "SELECT * FROM sps_afiliados WHERE afiliado_credenciamento='Sim' AND afiliado_nacao='$pais' AND afiliado_estado != '' GROUP BY afiliado_estado");
  while($verLojas2 = mysqli_fetch_array($sqlLojas2)){
      $estado = $verLojas2['afiliado_estado'];
      
  $sqlEstado = mysqli_query($conexao, "SELECT * FROM sps_estado WHERE estado_sigla='$estado'");
  $verEstado = mysqli_fetch_array($sqlEstado);
  ?>
  	<option value="<?Php echo $verLojas2['afiliado_estado']; ?>"><?Php echo $verEstado['estado_nome']; ?></value>
  <?Php
  }
  ?>
  ...
  <input name="pais" type="hidden" value="<?Php echo $pais; ?>" /><br>
  
  <table style="width:100%;">
    <tr>
        <td style="width:50%; text-align:right;"><a href="javascript:history.back(-1);"><button type="button" class="w3-input w3-red btn-lg w3-padding-16" style="width:98%;"><i class="fas fa-reply"></i> Voltar</button></a></td>
