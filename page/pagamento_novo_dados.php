...
if($tipo == "1"){
    $codigo = "$linha1 $linha2 $linha3 $linha4 $linha5";
    $nomeTipo = "Boleto Diversos";
}elseif($tipo == "2"){
    $codigo = "$linha1 $linha2 $linha3 $linha4";
    $nomeTipo = "Água, Luz, Telefone, Gás";
}elseif($tipo == "3"){
    $codigo = "$linha1 $linha2 $linha3 $linha4 $linha5";
    $nomeTipo = "Cartão de Crédito ou Outros Pagamentos";
}

$sqlCodigo = mysqli_query($conexao, "SELECT * FROM sps_pagamentos WHERE pag_codigo='$codigo'");
$verCodigo = mysqli_fetch_array($sqlCodigo);


if($tipo == "1" && $codigo == $verCodigo['pag_codigo']){
    echo "<h2 class='w3-large'>Este Código já encontra-se cadastrado em nosso Banco de Dados</h2><br><br>";
}elseif($tipo == "2" && $codigo == $verCodigo['pag_codigo']){
    echo "<h2 class='w3-large'>Este Código já encontra-se cadastrado em nosso Banco de Dados</h2><br><br>";
}else{
?>

<div class="w3-text-gray w3-light-gray w3-padding w3-round w3-round" style="font-size:12px; margin-top:10px; margin-bottom:10px;"><b>Olá <?Php echo $name; ?></b><br>
<b>Tipo de Pagamento: </b><?Php echo $nomeTipo; ?>
</div>

<b>2º Passo:</b> Complete os dados abaixo<br><br>
<center><b>Código de Barras:</b><br>
<span style="font-size:11px"><?Php echo $codigo;?></span></center>
<form action="pagamento_novo_dados_confirmar.php?id=<?Php echo $id; ?>&&empresa=<?php echo $empresa; ?>" method="post" name="form">
    <input name="tipo" type="hidden" value="<?Php echo $tipo; ?>">
    <input name="codigo" type="hidden" value="<?Php echo $codigo;?>">
    <input type="number" step="0.010" name="valor" class="form-control input-lg" id="valorPagar2" placeholder="Valor do Boleto" style="text-transform:uppercase; margin-top:2px;font-family:Trebuchet MS, sans-serif;"  autocomplete="off"  required="required">
    <input name="data" placeholder="Data Vencimento"  class="form-control input-lg" style="text-transform:uppercase; margin-top:2px;font-family:Trebuchet MS, sans-serif;" id="inputdefault" onkeyup="Mascara('DATA',this,event)" maxlength="10" type="tel" autocomplete="off" required>
    <input type="text" name="descricao" class="form-control input-lg" style="text-transform:uppercase; margin-top:2px; margin-bottom:5px;font-family:Trebuchet MS, sans-serif;" placeholder="Descrição" autocomplete="no"  /> 
