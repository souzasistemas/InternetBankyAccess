...
require "../../../../../config/config.php";  
		...
		$arquivo = 'saques_banco_acessomundi_bank.xls';
		
		// Criamos uma tabela HTML com o formato da planilha
		$html = '<meta charset="utf-8">';
		$html .= '<table border="1">';
		$html .= '<tr>';
		$html .= '<td colspan="11" align="center"><b>Relatório de Saques Pendente - Modalidade TRANSFERÊNCIA BANCÁRIA</b></tr>';
		$html .= '</tr>';
		...
		$html .= '<td><b>Banco (Código Banco)</b></td>';
		$html .= '<td><b>Tipo de Conta</b></td>';
		...
		$resultado_msg_contatos = mysql_query("SELECT * FROM sps_extrato WHERE extrato_status_saque='Pendente' AND extrato_modo_saque='Transferência Bancária' ORDER BY extrato_id ASC");
		...
		while($row_msg_contatos = mysql_fetch_array($resultado_msg_contatos)){
			...
			$html .= '<td>'.$verbanco['banco_nome'].' ('.$idBanco.') </td>';
			$html .= '<td>'.$verAssociado['afiliado_tipoconta'].'</td>';
			$html .= '<td>'.$verAssociado['afiliado_agencia'].'</td>';
			$html .= '<td>'.$verAssociado['afiliado_conta'].'</td>';
