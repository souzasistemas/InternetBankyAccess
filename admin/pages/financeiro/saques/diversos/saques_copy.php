...
<h2 class="w3-xlarge">Relatório Saques Pendentes</h2>

<?Php
$codigo = $_POST['codigo'];

if($codigo == ""){
$busca = "SELECT * FROM sps_extrato WHERE extrato_status_saque='Pendente' ORDER BY extrato_id ASC"; 
...
$sqlTotal = mysql_query("SELECT sum(extrato_valor), extrato_id FROM sps_extrato WHERE extrato_status_saque='Pendente'");
    ...
    <tr>
        ...
        <td style="text-align:right;">
            ...
            <form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post" name="form">
                ...
                <div class="w3-tag w3-round w3-green" style="padding:3px">
                 <a href="gerar_excel.php"><button class="w3-tag w3-round w3-green w3-border w3-border-white" type="button"><i class="fa fa-file-excel-o" aria-hidden="true"></i> Gerar Excell</button></a>
                </div>
...
<tbody style="font-size:12px; cursor:pointer;">
    <tr>
        ...
        <td style="text-align:center; vertical-align: middle;">
            <?Php
        $descricao = "SOLICITAÇÃO DE SAQUE VIA ZENCARD";
        $descricao2 = "SOLICITAÇÃO DE SAQUE VIA TRANSFERENCIA BANCARIA";
        $idAfiliadoBanco = $produto['extrato_afiliado_id'];
        
        $sqlTotal2 = mysql_query("SELECT * FROM sps_extrato WHERE extrato_descricao='$descricao' AND extrato_status_saque='Pendente' AND extrato_afiliado_id='$idAfiliadoBanco'");
        $verTotal2 = mysql_fetch_array($sqlTotal2);
        
        if($verTotal2['extrato_id'] == ""){
        
        $sqlTotal3 = mysql_query("SELECT * FROM sps_extrato WHERE extrato_descricao='$descricao2' AND extrato_status_saque='Pendente' AND extrato_afiliado_id='$idAfiliadoBanco'");
        $verTotal3 = mysql_fetch_array($sqlTotal3);
		...
		$idBanco = $verSaqueAfiliado['afiliado_banco'];
		...
		?>
        <strong>
        <?php echo $verbanco['banco_nome']; ?> (<?php echo $idBanco; ?>)</strong> - CONTA <?php echo $verSaqueAfiliado['afiliado_tipoconta']; ?> <br>
        <strong>Agência:</strong> <?php echo $verSaqueAfiliado['afiliado_agencia_banco']; ?>&nbsp; 
        <strong>Conta:</strong> <?php echo $verSaqueAfiliado['afiliado_conta']; ?><br />
		<?Php
        }else{
            $idAfiliadoBanco = $produto['extrato_afiliado_id'];
...
$busca = "SELECT * FROM sps_extrato WHERE extrato_status_saque='Pendente' AND extrato_afiliado_id='$codigo' ORDER BY extrato_id ASC"; 
    ...
    <tr>
        ...
        <td style="text-align:right;">
            ...
            <form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post" name="form">
                ...
                <div class="w3-tag w3-round w3-green" style="padding:3px">
                 <a href="gerar_excel_codigo.php?id=<?Php echo $codigo; ?>"><button class="w3-tag w3-round w3-green w3-border w3-border-white" type="button"><i class="fa fa-file-excel-o" aria-hidden="true"></i> Gerar Excell</button></a>
                </div>
                <div class="w3-tag w3-round w3-deep-orange" style="padding:3px">
                 <a href="saques_banco.php"><button class="w3-tag w3-round w3-deep-orange w3-border w3-border-white" type="button"><i class="fa fa-bullseye" aria-hidden="true"></i> Filtrar por Banco</button></a>
                </div>
...
<h2 class="w3-xlarge">Relatório de Saques Pendentes</h2>
<?php
$busca = "SELECT * FROM sps_extrato WHERE extrato_status_saque='Pendente' ORDER BY extrato_id ASC"; 
