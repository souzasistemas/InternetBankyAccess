    ...
    <form action="<?php echo $_SERVER['PHP_SELF'] ?>?id=<?Php echo $idAdm; ?>" name="form" method="POST" class="botao">
        ...
        <table class="w3-table">
            <tr>
                ...
                <td colspan="5" style="vertical-align:middle;"><input name="idConta" type="number" class="w3-input w3-white w3-round w3-border" style="width:250px;" required></td>
            </tr>
            <tr>
...
$sql = mysql_query("SELECT * FROM sps_afiliados WHERE afiliado_id='$idConta'");
...
<div class="col-sm-12">
    <div class="well" style="padding:10px; vertical-align:middle; font-size:12px; height:100px;">
        ...
        <strong>Cidade/UF:</strong> <?Php echo strtoupper($ver['afiliado_cidade']); ?>/<?Php echo strtoupper($ver['afiliado_estado']); ?> &nbsp; &nbsp; &nbsp;<strong>CEP:</strong> <?Php echo strtoupper($ver['afiliado_cep']); ?>&nbsp; &nbsp; &nbsp; <strong>Conta:</strong> <?Php echo strtoupper($ver['afiliado_id']); ?>-<?Php echo strtoupper($ver['afiliado_codigo_verificador']); ?>
    </div>
...
/***** saldo em conta */
$sql_credito = mysql_query("SELECT sum(extrato_valor) FROM sps_extrato WHERE extrato_afiliado_id='$idConta' AND extrato_tipo='Credito'");
$ver_credito = mysql_fetch_assoc($sql_credito);
$credito = $ver_credito['sum(extrato_valor)'];

$sql_debito = mysql_query("SELECT sum(extrato_valor) FROM sps_extrato WHERE extrato_afiliado_id='$idConta' AND extrato_tipo='Debito'");
...
/***** limite aprovado  */
$sql_credito2 = mysql_query("SELECT sum(extrato_valor) FROM sps_extrato_credito WHERE extrato_afiliado_id='$idConta'");
$ver_credito2 = mysql_fetch_assoc($sql_credito2);
$credito2 = $ver_credito2['sum(extrato_valor)'];

$sql_movimento2 = mysql_query("SELECT sum(movimento_valor) FROM sps_movimentacao_credito WHERE movimento_afiliado_id='$idConta' AND movimento_status_afiliado='Pendente'");
...
/***** Débito em Aberto  */
$sql_debito2 = mysql_query("SELECT sum(pi_afiliado_id_valor) FROM sps_patrocinio WHERE pi_afiliado_id='$idConta' AND pi_status='Pendente'");
...
<table class="w3-table" style="font-size:12px;">
        ...
        $mesAtual = date('m');
        $anoAtual = date('Y');
$sqlExtrato = mysql_query("SELECT * FROM sps_extrato WHERE extrato_afiliado_id='$idConta' AND extrato_mes='$mesAtual' AND extrato_ano='$anoAtual' ORDER BY extrato_id DESC");
...
$sqlExtrato = mysql_query("SELECT * FROM sps_extrato WHERE extrato_afiliado_id='$idConta' AND extrato_dia >= '$num' AND extrato_dia <= '$num2' AND  extrato_mes='$mesAtual' AND extrato_ano='$anoAtual' ORDER BY extrato_id DESC");
while($verExtrato = mysql_fetch_array($sqlExtrato)){
?>
...
<table class="w3-table" style="font-size:12px;">
        ...
        <?Php
$sqlExtrato = mysql_query("SELECT * FROM sps_extrato WHERE extrato_afiliado_id='$idConta' AND extrato_mes='$mes' AND extrato_ano='$ano' ORDER BY extrato_id DESC");
while($verExtrato = mysql_fetch_array($sqlExtrato)){
?>
